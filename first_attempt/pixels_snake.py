...
def main():
    ...
    # snake = Brain((400, 4), activation_function='sigmoid', cost_function='quadratic', x=starting_x, y=starting_y)

    with open('trained_snake_pixels.pkl', 'rb') as f:
        snake = dill.load(f)

    snake.learning_rate = 0.1
    ...
    while True:
        ...
        '''
        if counter % 10000 == 0:
            snake.save_as('trained_snake_pixels')
    ...
    for i in range(boxes_per_row):
        for j in range(boxes_per_column):
            if display_surface.get_at_mapped((i * box_size + 2, j * box_size + 2)) == -256:
                ...
                pixels.append(3)
            elif display_surface.get_at_mapped((i * box_size + 2, j * box_size + 2)) == 65280:
                pixels.append(2)
            else:
